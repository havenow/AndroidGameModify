...
bool valtostr(const value_t * val, char *str, size_t n)//handler__list handler__watch
{
    char buf[128];
    int max_bytes = 0;
...
void uservalue2value(value_t *dst, const uservalue_t *src)//handler__set
{
...
void truncval_to_flags(value_t * dst, match_flags flags)//handler__list checkmatches
{
    assert(dst != NULL);
...
void truncval(value_t * dst, const value_t * src)//handler__watch
{
    assert(src != NULL);
...
void valnowidth(value_t * val)//peekdata searchregions
{
    assert(val);
...
bool parse_uservalue_bytearray(char **argv, unsigned argc, bytearray_element_t *array, uservalue_t *val)//handler__default handler__write
{
    int i,j;
...
bool parse_uservalue_number(const char *nptr, uservalue_t * val)//handler__set handler__decinc handler__default
{
...
bool parse_uservalue_int(const char *nptr, uservalue_t * val)//parse_uservalue_number
{
    int64_t num;
    char *endptr;
...
bool parse_uservalue_float(const char *nptr, uservalue_t * val)//parse_uservalue_number
{
    double num;
    char *endptr;
    assert(nptr);
...
int flags_to_max_width_in_bytes(match_flags flags)//val_max_width_in_bytes handler__set handler__list nth_match delete_by_region
{
...
/*
unsigned char get_uchar (const value_t * val)
{
	 if (sizeof(unsigned char) <= 1) return (unsigned char)get_u8b(val);
else if (sizeof(unsigned char) <= 2) return (unsigned char)get_u16b(val);
else if (sizeof(unsigned char) <= 4) return (unsigned char)get_u32b(val);
else if (sizeof(unsigned char) <= 8) return (unsigned char)get_u64b(val);
else assert(false);
}

signed char get_schar (const value_t * val)

unsigned short get_ushort (const value_t * val)
signed short get_sshort (const value_t * val)

unsigned int get_uint (const value_t * val)
signed int get_sint (const value_t * val)

unsined long get_ulong (const value_t * val)
sined long get_slong (const value_t * val)

unsined long long get_ulonglong (const value_t * val)
sined long long get_slonglong (const value_t * val)
*/