...
//	#define DEBUG_PRINT(format,args...)
//#endif
//#define DEBUG_PRINT(format,args...) 0
...
void* get_remote_addr( pid_t target_pid, const char* module_name, void* local_addr )
{
	...
	LOGE( "[+] get_remote_addr: local[%x], remote[%x]\n", local_handle, remote_handle );
...
int inject_remote_process( pid_t target_pid, const char *library_path, const char *function_name, void *param, size_t param_size )
{
	...
	fprintf(stderr, "[+] Injecting process: %d\n", target_pid );
	...
	fprintf(stderr,"[+] Remote mmap address: %x\n", mmap_addr );
	...
	fprintf(stderr,"[+] Calling mmap in target process.\n" );
	...
	fprintf(stderr,"[+] Target process returned from mmap, return value=%x, pc=%x \n", regs.ARM_r0, regs.ARM_pc );
	...
	fprintf(stderr,"[+] Get imports: dlopen: %x, dlsym: %x, dlclose: %x\n", dlopen_addr, dlsym_addr, dlclose_addr );
	...
	fprintf(stderr,"[+] Inject code start: %x, end: %x\n", local_code_ptr, &_inject_end_s );
	...
	fprintf(stderr,"[+] _dlopen_param1_s: %x\n", _dlopen_param1_s );
	...
	fprintf(stderr,"[+] _dlsym_param2_s: %x\n", _dlsym_param2_s );
	...
	fprintf(stderr,"[+] _saved_r0_pc_s: %x\n", _saved_r0_pc_s );
	...
	fprintf(stderr,"[+] _inject_function_param_s: %x\n", _inject_function_param_s );

	fprintf(stderr,"[+] Remote shellcode address: %x\n", remote_code_ptr );
...
int main(int argc, char** argv) {
	pid_t target_pid = find_pid_of("com.khg.actionsquad");
