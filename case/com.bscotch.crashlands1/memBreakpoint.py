...
session = device.attach("com.bscotch.crashlands1")
...
function set_read_write_break()
{
	...
	Process.setExceptionHandler(function(exp) {//当写内存的时候进入Handler，打印相关信息之后，将内存属性改为可读可写
		...
		if (true/*exp.memory.address == 0xC1272624*/){
		//if (true){
			console.log('address libyoyo.so base:               ' + soAddr);//app运行的时候，libyoyo.so在系统中的基地址
			console.log('address where the exception occurred: ' + exp.address);//发生写内存时，哪一行代码做了写操作
			console.log('address where called in libyoyo.so   : ' + ptr(exp.address - soAddr));//根据这个地址偏移，可以在IDA中跳转到libyoyo.so对应的行
		...
		Memory.protect(ptr('0xC1272624'), 4, 'rw-');
	...
	/*console.log('set one time watchpoint', JSON.stringify({
	 ...
	 Memory.protect(ptr('0xC1272624'), 4, 'r--');//注意这里是r--，而不是r-x，因为这个内存块的属性是可读可写，没有可执行
